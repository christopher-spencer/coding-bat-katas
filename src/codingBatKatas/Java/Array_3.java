...
//    the same group appears in reverse order. For example, the largest mirror section in {1, 2, 3, 8, 9, 3, 2, 1} is length
    ...
    public int maxMirror(int[] nums) {
        ...
        for (int i = 0; i < nums.length; i++) {
            for (int j = nums.length - 1; j >= 0; j--) {
                ...
                while (mirrorMirror < nums.length && onTheWall >= 0
                        && nums[mirrorMirror] == nums[onTheWall]) {
                    currentMirrorLength++;
                    mirrorMirror++;
                    onTheWall--;
                }
                maxMirrorLength = Math.max(maxMirrorLength, currentMirrorLength);
            }
        }
